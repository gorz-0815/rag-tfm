...
def build_pdf(pages_lines: list[list[str]]) -> bytes:
    ...
    objects_by_id = {}
    objects_by_id[catalog_id] = f"{catalog_id} 0 obj\n<< /Type /Catalog /Pages {pages_root_id} 0 R >>\nendobj\n"
    objects_by_id[pages_root_id] = (
        f"{pages_root_id} 0 obj\n<< /Type /Pages /Kids [{kids}] /Count {len(page_obj_ids)} >>\nendobj\n"
    )
    objects_by_id[font_obj] = f"{font_obj} 0 obj\n<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>\nendobj\n"

    for lines, page_id, content_id in zip(pages_lines, page_obj_ids, content_obj_ids):
...
if __name__ == "__main__":
    ...
    out_path = sys.argv[1] if len(sys.argv) > 1 else "data/manuals/aquaflow-200-manual.pdf"
    pages = wrap_text()
