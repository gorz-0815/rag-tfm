...
from src.query import SIMILARITY_TOP_K, ask_full_doc, ask_no_context, ask_rag
...
def write_results(rows: list[dict], scores: dict) -> None:
    ...
    def avg_tokens(rows, usage_key, token_key):
    ...
    rag_input_tokens = avg_tokens(rows, "rag_usage", "input_tokens")
    rag_output_tokens = avg_tokens(rows, "rag_usage", "output_tokens")
    full_doc_input_tokens = avg_tokens(rows, "full_doc_usage", "input_tokens")
    full_doc_output_tokens = avg_tokens(rows, "full_doc_usage", "output_tokens")
    baseline_input_tokens = avg_tokens(rows, "no_context_usage", "input_tokens")
    baseline_output_tokens = avg_tokens(rows, "no_context_usage", "output_tokens")
