...
Also worth evaluating alongside technique selection (raised in PR #8 review, `results/sample_trace.md`): whether `SIMILARITY_TOP_K` (currently a fixed constant of 4 in `src/query.py`) should be tunable, or even reduced as low as 1, rather than always retrieving the same fixed count regardless of technique or document. Relates to but is distinct from the `dynamic-chunking-by-doc-size` stub, which is about chunk size/overlap scaling with document size - this is about whether top-k itself should vary (by technique, by document, or just be configurable).
