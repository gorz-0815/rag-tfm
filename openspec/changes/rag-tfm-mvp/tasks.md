...
- [x] 6.6 Verify: `python -m src.eval` completes end-to-end and produces a results table with a visible RAG-vs-no-context gap on manual-specific questions — verified 2026-08-23 against `data/manuals/aquaflow-200-manual.pdf`: faithfulness 0.969 (RAG) vs. 0.073 (no-context), answer relevancy 0.74 vs. 0.107, context precision 0.616, context recall 0.889; full-doc scored alongside as an extra (non-spec) data point at 0.981 faithfulness, on par with RAG. Per-question wall-clock latency was also captured: no-context was slowest (2.85s avg) despite doing the least work, because its answers ran ~2.7x longer without a "concise, context-only" system prompt constraining it; full-doc (1.9s) wasn't slower than RAG (2.22s excluding a one-time embedding-model load on the first call) despite sending the whole manual, since input-token prefill isn't the latency bottleneck. See `results/eval_results.md`.
