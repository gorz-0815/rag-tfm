...
- **Should `comparative-eval` (Section 6) score `--full-doc` as a third condition, not just RAG vs no-context?** Resolved during Section 6 implementation: no. The `comparative-eval` spec explicitly scopes to two conditions (no-context baseline vs RAG), and `--full-doc` sending the entire manual as context makes Ragas' context-precision/recall metrics (which measure retrieval quality) not meaningfully applicable — there's no retrieval step to score. A three-way comparison stays a future idea, not pulled into this change.
