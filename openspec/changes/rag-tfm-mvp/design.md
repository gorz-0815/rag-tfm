...
**Ragas judge model: `langchain-anthropic`'s `ChatAnthropic`, wrapped via Ragas' `LangchainLLMWrapper`; same local HF embeddings wrapped via `LangchainEmbeddingsWrapper` for context-based metrics.** Reuses the same model family as generation for consistency and avoids adding a third provider just for evaluation. **Two implementation-time compat issues, both in `src/eval.py`:** (1) the installed `langchain-community` (>=0.4, "sunset" per its own deprecation notice) dropped `chat_models.vertexai`, but `ragas` 0.4.x still imports `ChatVertexAI` from it at module load time purely for an isinstance check this project never hits (Anthropic-only) — worked around with a `sys.modules` stub installed before importing `ragas`. (2) `ragas.embeddings.LangchainEmbeddingsWrapper` needs a langchain-shaped `embed_query`/`embed_documents` interface, which neither llama_index's `HuggingFaceEmbedding` nor a second `langchain_community.embeddings.HuggingFaceEmbeddings` instance provided cleanly (the latter re-downloads the model under a different cache layout, which fails offline) — worked around with a small adapter class wrapping the already-loaded llama_index embedding model instance instead of loading a second copy.
