...
**Ragas judge model: `langchain-anthropic`'s `ChatAnthropic`, wrapped via Ragas' `LangchainLLMWrapper`; same local HF embeddings wrapped via `LangchainEmbeddingsWrapper` for context-based metrics.** Reuses the same model family as generation for consistency and avoids adding a third provider just for evaluation. **Two implementation-time compat issues:** (1) `ragas` 0.4.x imports `ChatVertexAI` from `langchain_community.chat_models.vertexai` at module load time purely for an isinstance check this project never hits (Anthropic-only) — that submodule was dropped from `langchain-community` >=0.4 ("sunset" per its own deprecation notice), so `requirements.txt` pins `langchain-community<0.4` (still compatible with the rest of the stack's `langchain-core` requirement) rather than working around the missing import in code. (2) `ragas.embeddings.LangchainEmbeddingsWrapper` needs a langchain-shaped `embed_query`/`embed_documents` interface, which neither llama_index's `HuggingFaceEmbedding` nor a second `langchain_community.embeddings.HuggingFaceEmbeddings` instance provided cleanly (the latter re-downloads the model under a different cache layout, which fails offline) — worked around with a small adapter class (`src/eval.py`'s `_LlamaIndexEmbeddingsAdapter`) wrapping the already-loaded llama_index embedding model instance instead of loading a second copy.

**Comparative eval's faithfulness scoring judges both conditions against RAG's retrieved contexts, not "no context" for the baseline.** The no-context answer never saw those chunks; scoring it against them (rather than against nothing) shows how much it invented versus what the manual actually documents, which is the actual point of the RAG-vs-baseline comparison. `context_precision`/`context_recall` stay RAG-only since they require retrieval to have happened. See `src/eval.py`'s module docstring.
...
- **Should `comparative-eval` (Section 6) score `--full-doc` as a third condition, not just RAG vs no-context?** Resolved during Section 6 implementation: partially yes. The `comparative-eval` spec's required scope stays two conditions (no-context baseline vs RAG) — `context_precision`/`context_recall` remain RAG-only since full-doc has no retrieval step to score. But faithfulness and answer_relevancy need only an answer and *a* context to judge, and full-doc has both (the whole manual text), so `src/eval.py` scores full-doc alongside as an extra, non-spec-required data point: each condition's faithfulness is judged against what it was actually given (RAG against its retrieved chunks, full-doc against the full manual text), isolating retrieval's cost from generation quality. Result: full-doc and RAG land within 0.01 of each other on faithfulness for this manual, so RAG's much smaller per-query token cost buys nearly the same accuracy — the actual trade-off the README's cost/latency section should make explicit.
