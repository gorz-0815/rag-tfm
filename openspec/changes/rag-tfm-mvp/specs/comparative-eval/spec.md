...
The evaluation command SHALL run every question in the eval set through both the no-RAG baseline and the RAG pipeline.

#### Scenario: Both conditions produce answers for every question
- **WHEN** the evaluation command completes
- **THEN** there is a recorded answer for every eval question under both the no-RAG condition and the RAG condition
...
- **THEN** faithfulness and answer relevancy scores exist for both the no-RAG and RAG conditions, and context precision/recall scores exist for the RAG condition
